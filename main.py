...
def openDirectory():
    ...
    all_files = [directory_path + "/" + f for f in listdir(directory_path) if isfile(directory_path + "/" + f) and f.lower().endswith(('.jpeg','.jpg'))]
    return all_files
...
class NewWindow(QDialog):
    def __init__(self):
        ...
        button = QPushButton("następny obrazek")
        button2 = QPushButton("edycja metadany")
        button3 = QPushButton("cofnij obrazek")
        label = QLabel()
        pixmap = QPixmap(tab1[0])
        scaled_pixmap = pixmap.scaled(600, 400)
        label.setPixmap(scaled_pixmap)
        layout1.addWidget(button3,4,1, alignment=Qt.AlignmentFlag.AlignCenter)
        layout1.addWidget(button, 4,2, alignment=Qt.AlignmentFlag.AlignCenter)
        layout1.addWidget(label,4,0, alignment=Qt.AlignmentFlag.AlignCenter)
        layout1.addWidget(button2,4,3, alignment=Qt.AlignmentFlag.AlignCenter)
        self.b=1
        def change_image():
        ...
        button3.clicked.connect(change_image2)
...
class NewWindow3(QDialog):
    def __init__(self, tab1, b):
        ...
        def zast():
               ...
               data=metadana4.text()
               data=data.split('.')
               data_wykonania = datetime(year=int(data[0]), month=int(data[1]), day=int(data[2]), hour=int(data[3]), minute=int(data[4]), second=int(data[5]))
               my_image.datetime_original=data_wykonania.strftime(DATETIME_STR_FORMAT)
               
               data2=metadana5.text()
               data2=data2.split('.')
               data_s= datetime(year=int(data2[0]), month=int(data2[1]), day=int(data2[2]), hour=int(data2[3]), minute=int(data2[4]), second=int(data2[5]))
